...
export default function Home(){
  ...
  return <div className="center">
    <div className="card">
      ...
      {!admin && <>
        {step==='search' && <>
          <input placeholder="Search your name..." value={search} onChange={e=>setSearch(e.target.value)} />
          {filtered.map(g=>
            <div key={g.id}>
              <label><input type="checkbox" onChange={e=>{
                if(e.target.checked)setSelected([...selected,g]);
                else setSelected(selected.filter(x=>x.id!==g.id));
              }} /> {g.name}</label>
            </div>
          )}
          {!!selected.length && <button onClick={()=>{
            let s={...tempStatus},d={...tempDiet};
            selected.forEach(g=>{
              const ex=respOf(g.name);
              s[g.id]=ex?.status||'';
              d[g.id]=ex?.dietary||'';
            });
            setTempStatus(s);setTempDiet(d);setStep('details');
          }}>Continue</button>}
        </>}

        {step==='details' && selected.map(g=>
          <div className="card-sm" key={g.id}>
            <b>{g.name}</b>
            <div className="row">
              <button className={'accept '+(tempStatus[g.id]==='Attending'?'active':'')}
                onClick={()=>setTempStatus({...tempStatus,[g.id]:'Attending'})}>Accept ✓</button>
              <button className={'decline '+(tempStatus[g.id]==='Declined'?'active':'')}
                onClick={()=>setTempStatus({...tempStatus,[g.id]:'Declined'})}>Decline ✗</button>
            </div>
        ...
        {step==='details' && <button onClick={async()=>{
          await Promise.all(selected.map(g=>
            fetch('/api/upsert',{method:'POST',headers:{'Content-Type':'application/json'},
              body:JSON.stringify({name:g.name,status:tempStatus[g.id],dietary:tempDiet[g.id]})})
          ));
          setStep('thanks');load();
        }}>Submit RSVP</button>}
        ...
        {step==='thanks' && <>
          ...
          <button onClick={()=>{setSelected([]);setStep('search')}}>RSVP Another Guest</button>
        </>}
      </>}
